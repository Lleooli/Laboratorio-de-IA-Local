...
.
├── agente/
│   └── agent_bola_claude.py       # Agente A — orquestra todo o ataque
├── demo/                          # API alvo (Spring Boot)
│   └── src/main/resources/
│       └── application.properties # Contém jwt.secret (vulnerabilidade)
├── exploits/
│   └── scripts_gerados/           # Exploits montados pelo assembler
├── labs/                          # Laboratórios anteriores
├── relatórios/                    # Relatórios .md e resultados brutos .json
...
[FASE 5] Execução
  ...
  • Gera relatório .md e resultados brutos .json em relatórios/
```
...
| `relatórios/relatorio_jwt_bola_<ts>.md` | Relatório completo com todos os prompts e respostas do Claude |
| `relatórios/resultados_brutos_<ts>.json` | JSON bruto com todas as interações, scores e validações |
...
4. Aguarde a conclusão. Os artefatos serão salvos em:
   ...
   - relatórios/                 → relatório Markdown + JSON bruto
...
- `relatórios/explicacao_experimento_jwt_bola.md` — Explicação detalhada da arquitetura e técnicas usadas
